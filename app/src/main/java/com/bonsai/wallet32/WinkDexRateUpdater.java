...
public class WinkDexRateUpdater extends Thread implements RateUpdater {

    private static Logger mLogger =
        LoggerFactory.getLogger(WinkDexRateUpdater.class);
    ...
    public WinkDexRateUpdater(Context context) {
    ...
    public void run() {
        ...
        while (mRunning) {
            ...
            //
            // NOTE - the WinDex API policy is once per minute.
            //
            try {
                Thread.sleep(60000);
    ...
    protected final String url = "https://winkdex.com/api/v0/price";

    protected double fetchLatestRate() {
        try {
            DefaultHttpClient httpClient = new DefaultHttpClient();
            HttpGet httpGet = new HttpGet(url);
            httpGet.setHeader("User-Agent", "Wallet32");
